...
//
// This worker polls a PLT source (currently a wallet-proxy-backed Concordium
// source) and writes normalized PLT transfer events into Postgres via the
// crp_plt_events table (using src/store/plt.pg.ts).
//
// The Concordium integration is intentionally minimal right now; it uses
// wallet-proxy /v3/accTransactions as its data source, tracking progress via
// wallet-proxy transaction ids.

// Load .env so CRP_STREAM_* and WALLET_PROXY_* variables are available
// when running via `npm run crp:worker:demo`.
...
function loadWorkerConfigFromEnv(): WorkerConfig {
  ...
  const maxTicks = Number(
    process.env.CRP_STREAM_MAX_TICKS ?? DEFAULT_MAX_TICKS
  );
  const tokenId =
  ...
  const decimals = Number(
    process.env.CONCORDIUM_PLT_DECIMALS ?? DEFAULT_DECIMALS
  );
  const network =
  ...
  const lastHeight = Number(
    process.env.CRP_STREAM_LAST_HEIGHT ?? 0
  );
...
async function runWorker(
  ...
  while (true) {
    ...
    if (cfg.maxTicks > 0 && tick > cfg.maxTicks) {
      console.log(
        "[CRP-STREAM] maxTicks (%d) reached, stopping loop.",
        cfg.maxTicks
      );
      break;
    }
    ...
    const { events, bestHeight } = await source.fetchSince(
      state.lastHeightExclusive
    );

    console.log(
      "[CRP-STREAM] fetched %d PLT event(s) above height %d",
      events.length,
      state.lastHeightExclusive
    );
    ...
    if (events.length > 0) {
      ...
      if (!cfg.dryRun) {
        ...
        console.log(
          "[CRP-STREAM] dryRun=1, would insert rows:",
          rows.length
        );
      }
    }
